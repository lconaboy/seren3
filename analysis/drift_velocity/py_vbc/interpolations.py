...
def interpolate_tf(flag, z, dz=None):
    ...
    z=zstart-dz. From these the derivative of the transfer function
    around z=zstart is calculated, and an interpolation is generated
    using cubic splines.

    :param flag: (str) 'c' for dark matter, 'b' for baryons
    :returns: tfz_spline -- the spline of the TF at z=zstart, dtf_spline -- the spline of the derivative of the TF around z=zstart
    :rtype: splines, which are used by supplying them with a a range of x values like y_interp = spline(x)
    """
 
    if ~np.any([flag=='c', flag=='b', flag=='g', flag=='t']):
        raise Exception('Flag should be either "c", "b", "g" or "t".')

    # Dict to convert flag to a TF index
    i_flag = {'c':1, 'b':2, 'g':3, 't':6}
    ...
    if z == 0: 
        ...
        # Load in TFs, skiprows=1 is an artefact of CICsASS
        tf0 = np.loadtxt(fn0)
        ...
        if dz is not None:
            ...
            # Load in TFs, skiprows argument is due to CICsASS requiring that
...
def interpolate_recfast():
     # skiprows is an artefact of cicsass
