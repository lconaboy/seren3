...
from main import interpolate_tf
...
def plot_comparison(x0, yc0, yb0, x1, yc1, yb1, d='', yl=False):
    ...
    tf_legend = [Line2D([0], [0], color='m', linestyle='-'),
                 ...
                 Line2D([0], [0], color='k', linestyle='--'),]
    tf_labels = ['{}T$_\mathrm{{k, c}}$'.format(d), '{}T$_\mathrm{{k, b}}$'.format(d),
                 'py\_vbc', 'CICsASS']
...
c_spline, dc_spline = interpolate_tf('c')
b_spline, db_spline = interpolate_tf('b')
...
# Convert k from CICsASS to k/h
hconst = 0.71  # The value of h used for this test
lk_c = np.log10(k_c0/hconst)
