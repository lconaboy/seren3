...
from py_vbc.test_constants import zinit, zstart, dz, vstream
...
def plot_comparison(x0, yc0, yb0, yc1, yb1):
    ...
    ax0.plot(x1, yc1, color='seagreen', linestyle='--')
    ax0.plot(x1, yb1, color='orange', linestyle='--')
...
g_pv = calc_derivs(g_c[:, 0], vstream, zstart, zinit, dz)

plot_comparison(g_c[:, 0], g_pv[:, 0], g_pv[:, 2], g_c[:, 0], g_c[:, 1], g_c[:, 2])
plt.show()
